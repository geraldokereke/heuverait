...
const Footer = () => {
  ...
  const linkClasses = "hover:text-[#41a7ad] transition-colors duration-200 text-xs md:text-[10px] xl:text-xs 2xl:text-sm";
  const headingClasses = "text-black font-bold mb-2 md:mb-4 text-xs md:text-[10px] xl:text-xs 2xl:text-sm";
            ...
            <div className="flex space-x-3">
              {socialLinks.map(({ Icon, href }, index) => (
                <a key={index} href={href} className={`${linkClasses} hover:text-[#41a7ad]`}>
                  <Icon className='size-4 md:size-3 xl:size-4 2xl:size-5' />
                </a>
              ))}
            </div>
