...
const LogoMarquee = () => {
    ...
    return (
        <div className='w-full py-20'>
            ...
            <motion.div
                className="flex items-center w-full grid grid-cols-4 gap-10 mt-12"
            >
                {/* First set of logos */}
                {companies.map((company, index) => (
                    <div
                        key={`first-${index}`}
                        className="mx-auto flex items-center justify-center w-20 xl:w-24 2xl:w-32 h-8 xl:h-12 2xl:h-20 mx-6 relative flex-shrink-0"
                    >
                        <Image
                            src={company.logo}
